...
//    }
//
//    var actualInitialization = new Function("if (document.getElementById('" + captchaId + "_CaptchaImage')) { window['" + captchaId + "'] = new BotDetect('" + captchaId + "', '" + instanceId + "', " + inputIdString + ", " + autoUppercaseInput + ", " + autoFocusInput + ", " + autoClearInput + ", " + autoReloadExpiredImage + ", " + autoReloadPeriod + ", " + autoReloadTimeout + ", " + soundStartDelay + ", " + limitSoundRegeneration + "); window['" + captchaId + "'].PostInit(); ; window['" + captchaId + "'].InitEventListeners(); }");
        ...
        // Dom Modifier - User Custom
        var captchaField = document.getElementById(captchaId + '_CaptchaField');
        if (captchaField == null) {
            console.warn("We recommend you using the customized HTML generator to hide our ADs");
        } else {
            var captchaDiv = document.getElementById(captchaId + '_CaptchaDiv');
            var captchaImageDiv = document.getElementById(captchaId + '_CaptchaImageDiv');
            var captchaIconsDiv = document.getElementById(captchaId + '_CaptchaIconsDiv');
            var captchaReloadLink = document.getElementById(captchaId + '_ReloadLink');
            var imageWidth = captchaField.getAttribute("imageWidth");
            var imageHeight = captchaField.getAttribute("imageHeight");
            captchaDiv.style.width = imageWidth + 'px';
            captchaDiv.style.height = imageHeight + 'px';
            captchaImageDiv.style.cursor = 'pointer';
            captchaImageDiv.style.height = imageHeight + 'px';
            captchaImageDiv.removeChild(captchaImageDiv.children[1]);
            captchaIconsDiv.style.display = 'none';
            captchaImageDiv.onclick = function () {
                captchaReloadLink.click();
            }
        }
    };
    ...
    BotDetect.UseHtml5Audio = function () {
        ...
        if (BotDetect.DetectAndroid() || BotDetect.DetectIOS()) {
            ...
            html5SoundSupported = (
                !!(browserCompatibilityCheck.canPlayType) && !!(browserCompatibilityCheck.canPlayType("audio/wav")) && !BotDetect.DetectIncompatibleAudio() // some browsers say they support the audio even when they have issues playing it
            );
        }
